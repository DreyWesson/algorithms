...
function rotateArr(arr, k) {
  ...
  if (typeof arr === "string" || typeof k === "string" || !Array.isArray(arr))
    return [];
  if (!k || !arr.length || isNaN(k)) return [];
  const memo = [];
  const tmp = [];
  for (let i = 0; i < arr.length; i++) {
    if (i < arr.length - k) memo.push(arr[i]);
    else tmp.push(arr[i]);
  }
...
// console.log(rotateArr([3, 8, 9, 7, 6], 3));
console.log(rotateArr([-4], 0));
